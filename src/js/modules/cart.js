  ...
  quantityCartHandler() {
    ...
    quantityButtons.forEach(button => {
      button.addEventListener('click', () => {
        let input = button.parentElement.querySelector('.c-quantity__input');
        let currentValue = parseInt(input.value);
        if (button.classList.contains('js-quantity-plus')) {
          input.value = currentValue + 1;
        } else if (button.classList.contains('js-quantity-minus') && currentValue > 1) {
          input.value = currentValue - 1;
        }
    ...
    quantityInputs.forEach(input => {
      input.addEventListener('change', () => this.#updateCartQuantity(input));
    });
  }

  #updateCartQuantity(input) {
    this.#cartChangeStatus();
    const productID = input.closest('.c-mini-cart-product').querySelector('.js-product-remove').getAttribute('data-product-id');
    const quantity = input.value;
    
    this.#fetchData('update_cart_quantity', { product_id: productID, quantity: quantity, lang: ajax.lang || 'en' })
      .then(data => {
        this.#cartChangeStatus();
